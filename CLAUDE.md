...
### 병렬 작업 (도메인 오너십)
- Game.gd 분리(2026-06-26) 후 콘텐츠 도메인이 자식 컨트롤러로 나뉨. **병렬 작업 시 그 도메인 영역만 건드린다.**
- **세션 시작 시 도메인만 말하면 됨**(예 "카드 작업") → [병렬작업/](./병렬작업/)의 해당 `.md`를 먼저 읽고 담당 파일·경계·`game.` 규칙·**커밋/푸시 주의** 파악
- 핵심: 자기 도메인 `.gd`만 수정 · 공유 상태는 `game.X` · 외부 노출 함수는 Game.gd facade 1줄 · `git add`는 자기 파일만(`-A` 금지) · 푸시 전 `pull --rebase`
...
| [병렬작업/](./병렬작업/) | 도메인별 병렬 작업 영역(담당 파일·경계·커밋규칙). 도메인 작업 시 먼저 확인 |

## 5. 알려진 부채
- `SkeletonWarrior._evolve()`/`evolves`/`kill_count`: MD6(진화 폐기)로 **제거 대상**
- ✅ `Game.gd` 분리 일단락(2026-06-26, ~1,832줄): 콘텐츠 도메인 6컴포넌트+데이터4 분리(병렬작업/). 잔여=인프라(VFX/HUD/튜토) 저우선. `Lobby.gd`(~486줄) 미착수
