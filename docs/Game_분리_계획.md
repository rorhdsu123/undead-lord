...
4. 🏁 **2026-06-26 일단락** — Game.gd 4,060→**1,832줄(−55%)**, 컴포넌트 6 + 데이터 4 분리. **콘텐츠 도메인(카드·상점·마물·강화·대사·웨이브) 전부 분리 = 라이브 서비스 목표 달성·클로버 해소.** 각 단계 플테OK·푸시 완료.

### 8.1 잔여(저우선 — 인프라, 재개 시 여기서)
콘텐츠와 무관(VFX/HUD/튜토는 자주 안 바뀜)·고결합(facade 다수)이라 비용 대비 효용 낮아 보류:
- **BattleFX**(VFX 20함수·흩어짐·facade ~15·무상태) — Game var 참조 0이라 기계적이긴 함.
- **ResourceHUD**(souls/readout — 액터+5컴포넌트 호출·facade 다수) · **Tutorial**(FTUE — `_is_tutorial`/`_close_guide` 도처 호출).
- **정적헬퍼 UIStyle**(스타일박스/바운스 — 무상태, 정적 클래스화 가능하나 호출처 repoint 다수).
- **코어(웨이브/성/보스/마왕/희생)는 허브에 잔존이 적절** — 분리 대상 아님.
- 재개 패턴: 메모리 `project_gamegd_split`의 검증 루틴(godot 직접로드·bare 메서드/const/var/self·다중줄시그니처 고아) 준수.

5. 각 추출 시 `아키텍처.md §3.2 책임표`·`§3.5 부채` 동반 갱신.
